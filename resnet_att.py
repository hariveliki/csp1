...
class DilatedBlock(nn.Module):
    def __init__(
        self, c_in=24, c_out=24, kernel_size=8, dilation_list=[1, 2, 4, 8], dp_rate=0.1
    ):
        ...
        for i, dilation in enumerate(dilation_list):
            if i == 0:
                layers.append(
                    ResUnit(
                        c_in, c_out, k=kernel_size, dilation=dilation, dp_rate=dp_rate
                    )
                )
            else:
                layers.append(
                    ResUnit(
                        c_out, c_out, k=kernel_size, dilation=dilation, dp_rate=dp_rate
                    )
                )
...
class ResNetAtt(nn.Module):
    ...
    def __init__(
        ...
        for i in range(n_dilated_block):
            layers.append(
                DilatedBlock(
                    c_in=in_channels if i == 0 else in_model_channel,
                    c_out=in_model_channel,
